...
var Mocks = require('./../../modules/mocks/mocks');


module.exports = function (app, passport, ig, config) {
    ...
    app.get('/api/campaigns/:id', passport.authenticate('jwt'), function (req, res) {
        ...
        Campaign.findOne({campaignId: req.params.id}).populate('participants')
    ...
    app.post('/api/campaigns/:id/status/:status', passport.authenticate('jwt'),
             function (req, res) {
            ...
            if (err) {
                ...
                if (alliance) {
                    if (err) {
                        ...
                        Campaign.update({campaignId: req.params.id},
                                        {$push: {participants: alliance._id}}, {},
                                        function (err, numAffected) {
                                            if (err) {
                                                ...
                                                if (numAffected.n > 0) {
                                                    Campaign.findOne({campaignId: req.params.id})
    ...
    app.get('/api/campaign/:id/tags/:tag', function (req, resp) {
        ...
        getTagInfo(req.params.tag, function (err, tagInfo) {
            if (err) {
    ...
    function getTagInfo(tag, cb) {

        if (config.mocks === true) {
            Mocks.getTagInfo(tag, cb);
        } else {
            ig.tag(tag, {access_token: result.access_token},
                   function (err, tagInfo, remaining, limit) {
                       console.log('Error', err);
                       cb(err, tagInfo);
                   });
        }
